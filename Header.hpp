...
public:
    ...
    ~MovieRoom();
    void deleteMovieInfo(MovieInfo* deleteInfo);  //영화 정보 삭제하면서 해당 관 내 상영영화 모두 삭제
...
public:
    ...
    void printTimetable(MovieRoom* room);  //영화관 선택 시 시작시간 순으로 상영영화 출력(상영 영화 추가 제거 화면)
    ...
    void deleteMoviePlay(MovieRoom* room, short selectTime);  //시작시간 정보로 해당 영화관 내 상영영화 삭제하기
    
    /* 유저 관련 함수 */
    Ticket* findTicket(Ticket* ticketTable, int tNumber)  ;     //유저 테이블에서 티켓번호로 User* 찾기
    void printTicket(int tNumber);                              //예매 번호로 티켓 정보 출력 (예매확인 창)
    void deleteTicket(int tNumber);                             //예매 번호로 티켓 삭제 (예매취소)
    void addTicket(MoviePlay* movie);                           //예매하기
